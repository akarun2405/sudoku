  ...
  for (i = (a / 3) * 3; i < (a / 3) * 3 + 3; i++)
    for (j = (b / 3) * 3; j < (b / 3) * 3 + 3; j++) {
      ...
      if (sq[i][j].no != 0) {
        ...
        new_end =
            remove(sq[a][b].prob.begin(), sq[a][b].prob.end(), sq[i][j].no);
        sq[a][b].prob.erase(new_end, sq[a][b].prob.end());
      }
    }
...
/*
This function basically checks for the presence of n boxes having exactly n same
possibility. If it finds such a occurence, it erases
      ...
      if (tempsize == temp.size()) {
        for (l = 0; l < 9; l++) {
          ...
          if (h < temp.size())
            continue;
          for (h = 0; h < temp.size(); h++) {
            new_end = remove(sq[a][l].prob.begin(), sq[a][l].prob.end(),
                             sq[a][j].prob[h]);
      ...
      if (tempsize == temp.size()) {
        for (l = 0; l < 9; l++) {
          ...
          if (h < temp.size())
            continue;
          for (h = 0; h < temp.size(); h++) {
            new_end = remove(sq[l][a].prob.begin(), sq[l][a].prob.end(),
                             sq[i][a].prob[h]);
...
//3***********************************************************//
/*
This function checks for the occurence of a possibility in exactly(FREQUENCY
CHECK) one box (either in column or row or the 3x3 box..
You may wonder that this might be a redundant test(I wondered),but this is kind
of different from other functions..
*/
...
//**********************************************FUNCTION TO CHECK THE COMPLETION
//OF MATRIX*********************************************//
...
int main() {
  ...
  string s;
  system("clear");
  cout << "Please enter the matrix as a number with blanks replaced by 0's"
       << endl;
  ...
  for (i = 0; i < 9; i++)
    for (j = 0; j < 9; j++) {
      if (sq[i][j].no == 0) {
        ...
        sq[i][j].prob.push_back(sq[i][j].no);
    }
  ...
  if (true) {
    while (incomplete(sq)) {
      ...
      // system("sleep 1");                               //Comment this
      // statement in windows..
      if (time > 35) {
        cout << endl
             << endl
             << " Sorry the program is unable to solve it.\nMaybe there is "
                "some guess required to solve the matrix further.\nOr that the "
                "initial problem was wrong....\n";
        break;
        is_complete = false;
      }
    }
    ...
    if (is_complete) {
      // system("clear");                                  //Change "clear" to
      // "cls" in windows..
      cout << "The completed matrix is as follows..." << endl;
      print_matrix(sq);
      cout << "Total time taken : " << time
           << endl; // The program doesn't really tale this much time.I am
                    // halting the system so
      // that people can watch the actual progress of the solution
      // process.../*:)*/
      // system("sleep 3");                                //Comment this
      // statement in windows..
    }
  }
  return 0;
